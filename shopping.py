    ...
    with open(filename) as f:
        ...
        for row in reader:
            evidence.append ([ 
                ...
                float(row[5]), # ProductRelated Duration
                float(row[6]), # BounceRates
                float(row[7]), # ExitRates
                float(row[8]), # PageValues
                float(row[9]), # SpecialDay
                datetime.strptime(row[10][:3],'%b').month - 1, # Month
                int(row[11]), # OperatingSystems
                int(row[12]), # Browser
                int(row[13]), # Region
                int(row[14]), # TrafficType
                1 if row[15] == "Returning_Visitor" else 0, # VisitorType 1 if row[16] ==
                1 if row[16] == 'TRUE' else 0, # Weekend
            ])
            label.append(1 if row[17] == 'TRUE' else 0) # Revenue
...
def evaluate(labels, predictions):
    ...
    true_positive = ((predictions == 1) & (labels == predictions)).sum()
    true_negative = ((predictions == 0) & (labels == predictions)).sum()
